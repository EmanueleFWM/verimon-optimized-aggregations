...
/*
  prop random : (!Exists x1 . ((P0 S (P P1)) & (x1 = ''34''))) 
*/

class Formula_random(monitor: Monitor) extends Formula(monitor) {
          
  val var_x1 :: Nil = declareVariables(("x1",false))
  ...
  override def evaluate(): Boolean = {
    ...
    // assignments5 (main formula excluding leaf nodes):
      ...
      now(3) = now(5).or(now(4).and(pre(3)))
      now(7) = relationToConstant("x1",EQOP,"34").or(pre(7))
      now(2) = now(3).and(now(7))
      now(1) = now(2).exist(var_x1.quantvar)
      now(0) = now(1).not()
  ...
  varsInRelations = Set("x1")
  val indices: List[Int] = List(3,5)

  pre = Array.fill(8)(bddGenerator.False)
  now = Array.fill(8)(bddGenerator.False)

  txt = Array(
    "!Exists x1 . ((P0 S (P P1)) & (x1 = ''34''))",
      "Exists x1 . ((P0 S (P P1)) & (x1 = ''34''))",
      "(P0 S (P P1)) & (x1 = ''34'')",
      "P0 S (P P1)",
      "P0",
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("P0","P1")

  formulae ++= List(new Formula_random(this))
}
