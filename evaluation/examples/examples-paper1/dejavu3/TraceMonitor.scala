...
/*
  prop random : !(Exists x1 . (x1 = ''24'' & !P3(x1))) 
*/
...
class Formula_random(monitor: Monitor) extends Formula(monitor) {
  ...
  override def evaluate(): Boolean = {
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(3) = relationToConstant("x1",EQOP,"24").or(pre(3))
      now(4) = now(5).not()
      now(2) = now(3).and(now(4))
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("P3")
