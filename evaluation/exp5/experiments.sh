...
REPETITIONS=1
FORMULAS="aggregation/monpoly;verimon;verimon-old"
...
function monitor_cmd() {
  ...
  if [ $monitor = "verimon-old" ]; then
      echo "./verimon_old -no_rw -nofilteremptytp -nofilterrel -nonewlastts -sig ${fma}.sig -formula ${fma}.mfotl -log ${log} -verified > /dev/null"
      return
  fi
